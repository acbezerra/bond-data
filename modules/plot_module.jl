...
function get_period(df::DataFrame, date_cols::Array{Symbol, 1})
    ...
    qtr = :trd_exctn_qtr in date_cols ? Int64(unique(df[:, :trd_exctn_qtr])[1]) : missing
    mo = :trd_exctn_mo in date_cols ? Int64(unique(df[:, :trd_exctn_mo])[1]) : missing
...
function get_plt_fpath_name(plt_type::String, stats_var::Symbol,
    ...
    small_trd_name = small_trades ? "_small_trd" : ""

    fname = string(period, "_", plt_type, "_",
                   stats_var, "_", y_var, dual_name,
                   small_trd_name)
